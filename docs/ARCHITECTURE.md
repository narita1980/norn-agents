...
## 5. ライブ配信プロトコル (SSE) と Human-in-the-loop
...
サーバ側は `norn.events.bus.EventBus` の in-memory pub-sub で配信。**uvicorn `--workers 1`** が前提で、複数プロセス運用は Redis Pub/Sub などへの差し替えが必要です。
