...
*   **セッションログイン**：ログイン ID/パスワードは `users` テーブル（bcrypt ハッシュ）のみ。`POST /auth/login` で JWT Cookie を発行し `/chat` `/reviews` `/dashboard` を常に保護（`SessionAuthMiddleware`）。UI は `LoginGate`。ユーザー作成は `uv run python -m norn.cli create-user`。`/webhook/*` `/healthz` `/readyz` `/auth/*` は除外。
