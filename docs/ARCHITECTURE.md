...
[Chat UI + SSE]     <──(REST/SSE)──   ├── urd / ウルド（技術）
                                      ├── verdandi / ヴェルダンディ（共感）
                                      ├── skuld / スクルド（未来）
                                      └── moderator / モデレーター（合議）
```
...
*   **Orchestration Pattern**：`urd` → `verdandi` → `skuld` → `moderator` の **固定逐次合議**（GroupChat / ラウンドロビンは使用しない）。UI 表示名は [CONVENTIONS.md](CONVENTIONS.md) 参照。
...
*   **画面**：`chat` / `about`（Norn とは）/ `dashboard` — 詳細は [FRONTEND.md](FRONTEND.md)。
...
| :--- | :--- | :--- | :--- |
| **1. 技術解析** | **ウルド（技術）** `urd` | コード差分を読み込み、バグ、脆弱性、パフォーマンス、設計規約違反を抽出。 | Ruff の出力をベースに、厳格なコードレビュープロンプトを適用。 |
| **2. 共感・トーン調整** | **ヴェルダンディ（共感）** `verdandi` | ウルドの指摘をレビューし、若手が挫折しないように「言い方」をマイルドに修正。 | 心理的安全性に基づくトーン調整 [4]。 |
| **3. キャリア・成長拡張** | **スクルド（未来）** `skuld` | 学習リソース・成長機会を提案。 | プロンプト生成（RAG は未実装 — §7）。 |
