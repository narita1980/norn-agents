...
*   **Basic 認証（任意）**：`NORN_BASIC_AUTH_USERNAME` / `NORN_BASIC_AUTH_PASSWORD` を両方設定すると、UI・API を HTTP Basic Auth で保護します（`BasicAuthMiddleware`）。`/webhook/*` と `/healthz` `/readyz` は除外。開発時は Vite の `configureServer` でも同じ env を読み込み 5173 側でも保護します。
